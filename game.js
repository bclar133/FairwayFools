...
let selectedGolfer=3,selectedDifficulty='medium',course=[],holeIndex=0,strokes=0,scores=[],ball={x:480,y:585},aim=0,club=0,phase='ready',power=0,accuracy=0,meterDir=1,animFrame,lastTime=0,hole,lie='TEE',shotAnimating=false,viewScale=1,pendingCupMessage='';
...
function drawBall(){const s=1/viewScale,r=5*s;ctx.fillStyle='rgba(0,0,0,.25)';ctx.beginPath();ctx.ellipse(ball.x+2*s,ball.y+4*s,7*s,3*s,0,0,7);ctx.fill();if(!shotAnimating&&topDownImg.complete&&topDownImg.naturalWidth){const cellW=topDownImg.naturalWidth/4,w=44*s,h=58*s;ctx.save();ctx.translate(ball.x,ball.y);ctx.rotate(aim);ctx.drawImage(topDownImg,selectedGolfer*cellW,0,cellW,topDownImg.naturalHeight,-w*.55,-h*.77,w,h);ctx.restore()}ctx.fillStyle='#fff';ctx.beginPath();ctx.arc(ball.x,ball.y,r,0,7);ctx.fill();ctx.strokeStyle='#203629';ctx.lineWidth=1*s;ctx.stroke()}
...
function hitBall(){
 ...
 const shotYards=clubDistance()*(.42+.58*power/100),pxPerYard=(hole.tee.y-hole.green.y)/hole.length,d=shotYards*pxPerYard;
 ...
 if(isPutt){tx+=Math.cos(hole.slope.angle)*hole.slope.strength*d*.14;ty+=Math.sin(hole.slope.angle)*hole.slope.strength*d*.14}
 if(isPutt){
  const target={x:tx,y:ty},pass=pathNearPoint(hole.green,start,target),overshoot=dist(target,hole.green),cupRadius=5.5;
  if(pass.t>.03&&pass.t<.99&&pass.distance<cupRadius){
   const lineQuality=1-pass.distance/cupRadius,paceQuality=Math.max(0,1-overshoot/(hole.green.r*.72));
   const sinkChance=.08+.82*lineQuality*paceQuality;
...
function landBall(previous){shotAnimating=false;const g=hole.green;if(dist(ball,g)<Math.max(7,g.r*.12)){ball={x:g.x,y:g.y};draw();if(pendingCupMessage)toast(pendingCupMessage);setTimeout(finishHole,650);return}if(hole.waters.some(w=>pointInEllipse(ball,w))){ball=previous;strokes++;toast('SPLASH! +1 PENALTY')}else if(hole.bunkers.some(b=>pointInEllipse(ball,b))){lie='BUNKER';toast('BEACH DAY')}else if(dist(ball,g)<g.r){lie='GREEN';club=7;toast(pendingCupMessage||'ON THE GREEN')}else if(onFairway(ball.x,ball.y)){lie='FAIRWAY';toast('FAIRWAY FOUND')}else{lie='ROUGH';toast('IN THE ROUGH')}pendingCupMessage='';phase='ready';power=0;accuracy=0;resetMeter();autoClub();updateHUD();draw()}
