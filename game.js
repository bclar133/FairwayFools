...
const puttModes=[['Short Putt',12],['Medium Putt',22],['Long Putt',36]];
let selectedGolfer=3,selectedDifficulty='medium',course=[],holeIndex=0,strokes=0,scores=[],ball={x:480,y:585},aim=0,club=0,puttMode=1,phase='ready',power=0,accuracy=0,meterDir=1,animFrame,lastTime=0,hole,lie='TEE',shotAnimating=false,viewScale=1,pendingCupMessage='',golferSwingPose=0;
...
function loadHole(){hole=course[holeIndex];ball={...hole.tee};strokes=0;lie='TEE';aimAtHole();club=hole.par===3?3:0;puttMode=1;phase='ready';power=0;accuracy=0;updateHUD();draw()}
function updateHUD(){
 ...
 $('#clubLabel').textContent=(lie==='GREEN'?puttModes[puttMode][0]:clubs[club][0]).toUpperCase();$('#distanceLabel').textContent=`${clubDistance()} yds max`;$('#aimDegrees').textContent=`${Math.round(aim*180/Math.PI)}°`;
}
function clubDistance(){const g=golfers[selectedGolfer];if(lie==='GREEN')return puttModes[puttMode][1];let n=clubs[club][1];if(club<2)n*=.9+g.stats.power*.035;if(lie==='ROUGH')n*=.82;if(lie==='BUNKER')n*=.62;return Math.round(n)}
function formatScore(n){return n===0?'E':n>0?`+${n}`:`${n}`}
function getCamera(){
 if(lie!=='GREEN')return {scale:1,x:0,y:0};
 const scale=Math.min(7,Math.max(3.8,260/hole.green.r));
 return {scale,x:480-hole.green.x*scale,y:320-hole.green.y*scale};
}
...
function drawGreen(){const g=hole.green,s=1/viewScale,flagH=lie==='GREEN'?46*s:37;ctx.fillStyle='#9bdd73';ctx.beginPath();ctx.arc(g.x,g.y,g.r,0,7);ctx.fill();ctx.strokeStyle='#b9ef91';ctx.lineWidth=4*s;ctx.stroke();ctx.fillStyle='#143c29';ctx.beginPath();ctx.ellipse(g.x,g.y+2*s,5*s,2.2*s,0,0,7);ctx.fill();ctx.strokeStyle='#fff';ctx.lineWidth=2*s;ctx.beginPath();ctx.moveTo(g.x,g.y);ctx.lineTo(g.x,g.y-flagH);ctx.stroke();ctx.fillStyle='#ef4a3d';ctx.beginPath();ctx.moveTo(g.x,g.y-flagH);ctx.lineTo(g.x+18*s,g.y-flagH+7*s);ctx.lineTo(g.x,g.y-flagH+14*s);ctx.fill();if(lie==='GREEN'){for(let y=g.y-g.r*.55;y<g.y+g.r*.55;y+=17)for(let x=g.x-g.r*.6;x<g.x+g.r*.6;x+=22){if(dist({x,y},g)<g.r*.72)drawSlopeArrow(x,y)}}}
...
function hitBall(){
 cancelAnimationFrame(animFrame);shotAnimating=true;strokes++;pendingCupMessage='';
 const g=golfers[selectedGolfer],isPutt=lie==='GREEN',start={...ball};
 const strength=isPutt?.05+.95*power/100:.42+.58*power/100;
 const shotYards=clubDistance()*strength,pxPerYard=(hole.tee.y-hole.green.y)/hole.length,d=shotYards*pxPerYard;
...
function landBall(previous){shotAnimating=false;const g=hole.green;if(dist(ball,g)<3.4){ball={x:g.x,y:g.y};draw();if(pendingCupMessage)toast(pendingCupMessage);setTimeout(finishHole,650);return}if(hole.waters.some(w=>pointInEllipse(ball,w))){ball=previous;strokes++;toast('SPLASH! +1 PENALTY')}else if(hole.bunkers.some(b=>pointInEllipse(ball,b))){lie='BUNKER';toast('BEACH DAY')}else if(dist(ball,g)<g.r){lie='GREEN';club=7;const remaining=dist(ball,g)/((hole.tee.y-hole.green.y)/hole.length);puttMode=remaining<9?0:remaining<19?1:2;toast(pendingCupMessage||'ON THE GREEN')}else if(onFairway(ball.x,ball.y)){lie='FAIRWAY';toast('FAIRWAY FOUND')}else{lie='ROUGH';toast('IN THE ROUGH')}pendingCupMessage='';phase='ready';power=0;accuracy=0;resetMeter();autoClub();aimAtHole();updateHUD();draw()}
