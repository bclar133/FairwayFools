...
function drawBall(){ctx.fillStyle='rgba(0,0,0,.25)';ctx.beginPath();ctx.ellipse(ball.x+2,ball.y+4,7,3,0,0,7);ctx.fill();ctx.fillStyle='#fff';ctx.beginPath();ctx.arc(ball.x,ball.y,5,0,7);ctx.fill();ctx.strokeStyle='#203629';ctx.lineWidth=1;ctx.stroke();if(!shotAnimating&&golferImg.complete&&golferImg.naturalWidth){const cellW=golferImg.naturalWidth/4,splitY=golferImg.naturalHeight*.325,sx=selectedGolfer*cellW;ctx.drawImage(golferImg,sx,splitY,cellW,golferImg.naturalHeight-splitY,ball.x-39,ball.y-103,58,104)}}
