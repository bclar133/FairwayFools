...
function drawTeeMarkers(){const t=hole.tee,a=Math.atan2(hole.green.x-t.x,t.y-hole.green.y),side={x:Math.cos(a),y:Math.sin(a)},back={x:-Math.sin(a)*5,y:Math.cos(a)*5},color=selectedDifficulty==='hard'?'#171b19':selectedDifficulty==='medium'?'#fff':'#f5c64d';[-1,1].forEach(n=>{const x=t.x+back.x+side.x*19*n,y=t.y+back.y+side.y*19*n;ctx.fillStyle='rgba(0,0,0,.3)';ctx.beginPath();ctx.ellipse(x+2,y+3,7,3,0,0,7);ctx.fill();ctx.fillStyle=color;ctx.beginPath();ctx.arc(x,y,5.5,0,7);ctx.fill();ctx.strokeStyle=selectedDifficulty==='hard'?'#808a84':'#315542';ctx.lineWidth=1.2;ctx.stroke();ctx.fillStyle='rgba(255,255,255,.45)';ctx.beginPath();ctx.arc(x-1.5,y-1.5,1.5,0,7);ctx.fill()})}
